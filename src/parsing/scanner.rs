            ...
            match self.read() {
                ...
                Some('\\') => {
                    ...
                    match self.read() {
                        ...
                        None => {
                            let text = "Expected an escape sequence".to_string();
                            return Token {
                                kind: TokenKind::Error(text),
                                span: Span { start, end: start },
                            };
...
mod tests {
    ...
    fn test_regex() {
        // letter
        let mut scanner = Scanner::init("[a-z]");
        scanner.read();
        assert_eq!(
            scanner.character_class(false),
            Token {
                kind: TokenKind::Regex("[a-z]".to_string()),
                span: Span { start: 0, end: 5 }
            }
        );
        assert_eq!(scanner.offset, 5);

        // the closing ] and the backslash
        // [\]\\]
        let mut scanner = Scanner::init("[\\]\\\\]");
        scanner.read();
        assert_eq!(
            scanner.character_class(false),
            Token {
                kind: TokenKind::Regex("[\\]\\\\]".to_string()),
